...
- **DONE 2026-07-20 — the DELIBERATION is on the new brain.** `chloe-adapter.mjs` → `lib/chloe-adapter.bundle.js`
  (esbuild, `build-adapter.mjs`, mirrors the affect build) now provides `window.ChloeBrain`/`ChloeCouncil`, replacing
  the inlined `brain.min.js` in the generator. `resolve()` — the vote tally that decides who speaks — delegates to the
  new brain's `council.js`. The shapes were already almost identical; the only rename is `winner` → `winnerId`.
  The rest of the contract (`createNervous`/`createCouncil`/`member`/`withTimeout`) is **pure orchestration with no
  neural content** — a pub/sub registry, a promise timeout, a propose→vote→veto loop — so it is reimplemented plainly
  in the adapter rather than dressed up as a migration.
  - **Parity proven:** `harness-adapter.js` (the 20-assertion validation harness, unmodified assertions) is green with
    the adapter injected, matching legacy 20-for-20.
  - **Verified in the real page:** all 6 globals present, `ChloeBrain` is the new adapter, `resolve` decides
    (warm wins by 2, consensus) and a veto strikes correctly (→ spark). A real chat turn renders with **0 JS errors**.
  - **Available but OFF:** the new brain's `moodTilt(tags, chem)` can weight a proposal by current chemistry, so a
    decision can be tilted by how she actually feels rather than static config weights. Opt-in via `opts.chem` —
    enabling it unconditionally would change every existing decision, and parity had to be provable first.
  - ⚠ **`../brain` is a DIRECTORY JUNCTION** to `D:\Claude\brain`, created because both build scripts assume the brain
    is a sibling (as `build-affect.mjs` already documented) and it was not. Rebuilds need it; remove with
    `rmdir "C:\Users\Fish\Documents\GitHub\brain"` (removes the link only, never the target).
